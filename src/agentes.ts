...
export const AGENTE_DEFS: Record<Agente, AgenteDef> = {
  analisador_contratos: {
    ...
    prompt: (e) =>
      [
        "Analise o contrato cujo texto está abaixo.",
  ...
  gerador_pecas: {
    ...
    prompt: (e) =>
      [
        ...
        `Fatos: ${s(e.fatos)}`,
        `Demandante (autor/requerente): ${s(e.demandante)}`,
        `Demandado (réu/requerido): ${s(e.demandado)}`,
        `Pedidos: ${s(e.pedidos)}`,
  ...
  resumidor_processos: {
    ...
    prompt: (e) =>
      [
        "Com base no texto dos autos/decisões/despachos abaixo, produza em markdown:",
        "1) Resumo executivo;",
  ...
  roteirista_social: {
    titulo: "Roteirista de Conteúdo",
    prompt: (e) => {
      const formato = s(e.formato) === "carrossel" ? "carrossel" : "reel";
      const entrega =
        formato === "carrossel"
          ? "Entregue como 5 a 7 slides numerados (1 ideia por slide)."
          : "Entregue como roteiro de fala, com indicação de cena/corte a cada bloco.";
      return [
        `Crie um roteiro de ${formato} para um advogado postar nas redes sociais.`,
        `Tema: ${s(e.tema)}`,
        "Estruture EXATAMENTE em 4 blocos rotulados, nesta ordem:",
        "1) HOOK — abertura que prende nos primeiros 3 segundos;",
        "2) DESENVOLVIMENTO — o conteúdo principal, útil e claro;",
        "3) AMPLIFICAÇÃO — reforço que aumenta alcance/desejo (ex.: dado, contraste, mito x verdade);",
        `4) CTA — chamada para ação: "${s(e.cta) || "link na bio"}".`,
        entrega,
        "Linguagem acessível ao público leigo. Respeite a ética da publicidade advocatícia:",
        "sem prometer resultado, sem captação/mercantilização e sem citar jurisprudência.",
      ].join("\n");
    },
  },
};
