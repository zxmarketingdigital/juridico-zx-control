...
// Entidades criadas NESTA migration (0001). `advogados` e tabelas de apoio
// vivem em 0002 e são cobertas por tests/schema-0002.test.ts.
const ENTIDADES_0001 = ["clientes", "casos", "documentos", "prazos", "pecas_geradas"] as const;
...
describe("schema.ts — constantes únicas (um valor, um lugar)", () => {
  ...
  it("são as entidades do spec §6 + Equipe + Growth/CRM", () => {
    expect([...TABLES].sort()).toEqual(
      ["advogados", "casos", "clientes", "custos", "documentos", "leads", "pecas_geradas", "prazos", "pre_notas", "receitas"].sort(),
    );
  });
});
